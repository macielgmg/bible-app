...
import { ArrowLeft, ArrowRight, BookOpen, Loader2, Highlighter as HighlighterIcon, Trash2, CheckCircle, XCircle, Eraser } from 'lucide-react'; // Removido Crown
...
import {
  ...
  DropdownMenuItem,
  DropdownMenuTrigger,
  DropdownMenuSeparator, // Adicionado para separar as opções
...
const ChapterDetail = () => {
  ...
  const [isErasingMode, setIsErasingMode] = useState(false); // Novo estado para o modo borracha
  const [showConfirmDeleteAllHighlights, setShowConfirmDeleteAllHighlights] = useState(false);
  // Removido: [showProFeatureModal, setShowProFeatureModal] = useState(false);

  // Verifica o query param 'highlight' na URL para iniciar no modo de marcação
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const highlightParam = params.get('highlight');
    if (highlightParam === 'true') {
      // Não há mais verificação de isPro, a funcionalidade é sempre ativada
      setIsHighlightingMode(true);
      setIsErasingMode(false); // Garante que o modo borracha não esteja ativo
    } else {
  ...
  const saveChapterProgress = useCallback(async (
    ...
    if (fetchError) { // maybeSingle() ainda pode retornar erros que não sejam PGRST116
  ...
  const fetchChapterAndStudyData = useCallback(async () => {
    ...
    try {
      ...
      // Removido: if (!studyData.is_free && !isPro) { ... }
      ...
      const { data: chapterDB, error: chapterError } = await supabase
        .from('chapters')
        .select('*, bible_text, explanation, application')
        .eq('id', chapterId)
        .eq('study_id', studyId)
        .single();
  ...
  const handleAdvance = async () => {
    ...
    if (success) {
      ...
      // Log user activity for chapter completion
      logUserActivity(session.user.id, 'chapter_completed', `Concluiu o capítulo ${chapterData.chapter_number} do estudo "${chapterData.study_title}".`);
  ...
  const handleSaveNotes = async () => {
    ...
    if (success) {
      ...
      logUserActivity(session.user.id, 'chapter_notes_saved', `Salvou anotações para o capítulo ${chapterData?.chapter_number} do estudo "${chapterData?.study_title}".`);
    }
  };
  ...
  const handleRemoveAllHighlights = async () => {
    ...
    try {
      ...
      logUserActivity(session.user.id, 'highlights_removed', `Removeu todas as marcações do capítulo ${chapterData?.chapter_number} do estudo "${chapterData?.study_title}".`);
      // Força a re-renderização do HighlightableText para limpar visualmente
  ...
  return (
    <div className="container mx-auto max-w-3xl">
        ...
        <CardContent className="space-y-6">
          <div className="prose max-w-none">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button 
                    ...
                    // Removido onClick para showProFeatureModal
                  >
            ...
            <HighlightableText 
              ...
              isErasingMode={isErasingMode} // Passa o novo prop
            />
              ...
              <AccordionContent className="p-4 pt-0">
                <div className="space-y-4">
                  <Textarea
                    ...
                    disabled={isAnyHighlightingModeActive} // Desabilita se qualquer modo de marcação estiver ativo
                  />
      ...
      {/* Removido: AlertDialog para recurso Pro */}
    </div>
  );
};
